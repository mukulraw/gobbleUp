...
/**
 * Created by hi on 8/4/2016.
 */

public class ProdAdapter3 extends AbstractRecyclerViewFooterAdapter<ProductBean>{
    ...
    public ProdAdapter3(Context context , RecyclerView recyclerView, ArrayList<ProductBean> dataSet, OnLoadMoreListener onLoadMoreListener , BottomSheetBehavior bar) {
    ...
    public void resetItems(@NonNull List<ProductBean> newDataSet) {
    ...
    @Override
    RecyclerView.ViewHolder onCreateBasicItemViewHolder(ViewGroup parent, int viewType) {
        View layoutView = LayoutInflater.from(parent.getContext()).inflate(R.layout.prod_list_model, null);

        return new ProdAdapter3.RecycleViewHolder(layoutView);
    }

    @Override
    void onBindBasicItemView(RecyclerView.ViewHolder genericHolder, int position) {

        final RecycleViewHolder holder = (RecycleViewHolder) genericHolder;
        final comparebean b = (comparebean)context.getApplicationContext();
        final ProductBean item = list.get(position);


        genericHolder.setIsRecyclable(false);
